...
const SyloSelect = () => {
  return (
    ...
    <div className="pt-6">
      ...
      <div className='pt-6 grid sm:grid-cols-4   overflow-x-auto w-auto  gap-4 transition-all'>

         {event_list.map((events) => (
          <div key={events.id} className={`${events.bgColor} relative shadow-lg rounded-2xl font-bold  text-xs md:text-lg min-w-30 h-28  flex justify-center items-center`}>
            <div >{events.name}</div>
